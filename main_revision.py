...
import time
from utils.functions_revision import reconnect
from utils.functions_revision import load_problem
import pprint as pp
    ...
    for batch_id, batch in tqdm(enumerate(dataloader), disable=opts.no_progress_bar):
        ...
        with torch.no_grad():
            ...
            if opts.problem_type in ['tsp', 'pctsp']:
                ...
                print(sum_cost(cost_ori, n_tsps_per_route).shape)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--gfn_loss', type=str, default='tb', help='The loss function for GFN')
    ...
    if opts.path == '':
        if opts.problem_type == 'tsp':
            ...
            opts.path = f'data/vrp/vrp{opts.problem_size}_128.pkl'

...
def trans_tsp(coors, routes, min_reviser_size=20):
    ...
    # print(f'padded_tsp_pis shape: {padded_tsp_pis.shape}')
    # print(f'padded_tsp_pis: {padded_tsp_pis[0]}')
    tsp_insts = coors[padded_tsp_pis]
    tsp_insts = torch.cat([tsp_insts, padded_tsp_pis.unsqueeze(-1)], dim=-1)
    assert tsp_insts.shape == (sum(n_tsps_per_route),max_tsp_len, 3)
    # print(tsp_insts.shape)
    return tsp_insts, n_tsps_per_route, max_tsp_len

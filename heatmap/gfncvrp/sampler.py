...
class Sampler():
    ...
    ##TODO: 점검해야함, 최대한 기존의 코드를 안 건드리는 선에서 method 새롭게 정의
    def off_cal_prob(self, trajectories):
        ...
        while not done:
            log_probs = None
            heatmap = self.heatmap[trajectories[:, tj_index]]  # current state heatmap 
            dist = (heatmap * visit_mask * capacity_mask * depot_mask)

            # prob calculation
            try:
                dist = Categorical(dist) # current state masked distribution
                log_probs = dist.log_prob(trajectories[:, tj_index + 1]) 
            except:
                dist = torch.softmax(torch.log(dist), dim=1)
                log_probs= torch.log(dist[torch.arange(self.bs), trajectories[:, tj_index + 1]])

            log_probs_list.append(log_probs)
            visit_mask = visit_mask.clone()
            depot_mask = depot_mask.clone()

            visit_mask = self.update_visit_mask(visit_mask, trajectories[:, tj_index + 1])
            used_capacity, capacity_mask = self.update_capacity_mask(trajectories[:, tj_index + 1], used_capacity)
            depot_mask, vehicle_count, demand_count = self.update_depot_mask(vehicle_count, demand_count, trajectories[:, tj_index+1], capacity_mask, visit_mask) 
            done = self.check_done(visit_mask, trajectories[:, tj_index + 1])
            tj_index += 1
        return torch.stack(log_probs_list).permute(1, 0)
